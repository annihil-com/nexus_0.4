...
void CNexUtils::getSettingsFromConf() {
	...
	sNex.useDet = config.ReadBoolean("Nexus", "detcheck", NEX_USEDET);
	sNex.useVer = config.ReadBoolean("Nexus", "vercheck", NEX_USEVER);
	#ifdef NEX_VERBOSE
		...
		log("Use DET: %i", sNex.useDet);
		log("Use VER: %i", sNex.useVer);
	#endif
